...
- **Every table is hybrid.** A field is a real column only when a query filters, sorts or joins on it or the database constrains it; everything else is in one JSON `data` column. `core/models/hybrid_document.py` is the mixin (`to_dict` / `from_dict` / `update_from_dict` / `to_json`), and `db.py` reads a row as one flat dict, changes the dict, and writes it back whole. Child rows ride along: a game's bag is rows of `items`, read and written as the game dict's `items` list. A new field on a fight is a key in the blob; a new or changed column is a migration.
- **The schema is versioned.** `PRAGMA user_version` marks a file; `connect_db()` creates a new file at `db.SCHEMA_VERSION` and refuses any other version with a 503. Each schema change adds the next numbered SQL script to `migrations/` and raises `SCHEMA_VERSION`; `tests/test_migrations.py` checks a migrated file matches a fresh one. Apply a script with `sqlite3 -bail <file> < migrations/NNN_*.sql`, after backing the file up.
...
- **The play-through is one row and its bag.** `Game` carries the hero's stats (columns `force`, `vie_max`, `vie_actuelle`, `gold`, never below zero), open combat, pending changes and death; its bag is `Item` rows, one per line. `db.py` changes and writes them whole. Anything that can drop Vie to zero must end by calling `db._lay_to_rest()`.
...
- **The book stays on disk.** SQLite holds only `page_views` (reading history), `games`, `items` and `page_inspections`. `DATABASE_DIR` names the directory and `APP_ENV` picks the file in it (`haute_tension_dev.sqlite3` / `_prod`); a new file gets its tables on first use, and a file an older schema wrote must be migrated first. A `DATABASE_DIR` that names a file is refused.
- **The log is part of the contract.** Every request leaves `Request` / `Answer` lines in `logs/general.log` via `application/logs/request_trace.py`; JSON answers are written in full, secrets are redacted by field name, `game_id` is cut to eight characters, which leaves an integer id whole. Use `note`/`event`/`failure` from `core.logs.general_log`, never `logging` or `print`.

## Tests

`tests/conftest.py` is the whole test infrastructure: `fake_db` (an in-memory SQLite per test, bound through the `connect_db()` seam), `books_path` (a tiny two-page book on disk; `books_path_with_fights` adds fights and conditional changes), `client` / `fighting_client` / `hero` (Flask test clients on top, the last with a hero already rolled). Seed data by assigning to `fake_db["games"].docs` — flat dicts in the `to_dict()` shape, columns and blob keys mixed, a game's bag riding along as its `items` list; reading `.docs` gives the same shape back. `tests/test_migrations.py` builds a version-0 file by hand and applies `migrations/` to it. Tests run against the real `logs/general.log` too, so lines from the suite in there are normal.
